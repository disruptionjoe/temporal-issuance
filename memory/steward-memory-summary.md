...
RUN-0062 adds: the cross-repo interface work is now organized into ten goals. G01 is the
...
RUN-0065 adds: G02 succeeded. A bounded `Compat_G^MLTT` finite trace passes through E067 with
verdict `source_issuance_candidate` plus `lossy_projection_residue`. This is a formal,
class-relative source fixture only; TI-C020 and physical-source status are unchanged. Next run:
G06 issued capability contract test using G03/G02 as paired baselines.
...
RUN-0070 adds: Goal 1 of the RSPS disproof ladder is now reproduced locally. The controlled-
copy / GHZ record-fidelity toy model selects the pointer basis exactly (`theta = 0`, normalized
`F = 3.0`) but does not derive Born weights. At the pointer basis, normalized fidelity is
constant across tested branch weights, while raw redundancy is `N * H2(p0)` and therefore
...
RUN-0080 adds: the minimal constructive witness is conditional but live. Finite-type and
infinite-computable contexts fail as source issuance. Fixed Platonist completion absorbs the
trace externally but fails as an internal witness. The survivor is local constructive context +
