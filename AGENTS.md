...
- A hard promotion — moving a claim to `promoted`, the tier an outside reader would take as "this repo asserts this is true" — MUST drop an evidence-trail awareness note in `../../../repos/private/system-runtime/mailboxes/system-attention/` using `agent-governance/templates/hard-promotion-joeops-notice.md`. The note is awareness-only, not an approval gate; the promotion is already done. Lower status moves (`speculative -> formalizing -> testable/weakened -> absorbed/killed/archived`) need no note.
...
- Needing a hostile or outside reviewer is a judgment input, not a work-stop. It shapes which path the steward chooses — prefer a path that does not depend on unavailable review, or proceed on the reviewed path and flag it — but it never halts the program and never triggers a global "no worthy work until reviewed" wait. Stopping all work to wait for an outside reviewer is a governance failure, not caution. When a path genuinely warrants outside review, drop a note in `../../../repos/private/system-runtime/mailboxes/system-attention/` and keep working.
...
- `primary_domain: nbl`
- Accepted relationship: `NBL-REL-004`.
...
- **Purpose:** Determine whether ongoing issuance is necessary to keep reality
  open to genuine novelty rather than collapsing into a closed, fully
  determined process.
- **Passion:** Test whether agents can turn a deep open-ended intuition into a
  disciplined research object without protecting it from honest failure.
- **Practice:** Help highly convicted clients turn powerful, underspecified
  intuitions into clear propositions that can be honestly tested, proved where
