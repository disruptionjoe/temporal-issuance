#!/usr/bin/env python3
"""Focused tests for dynamic/Floquet quantum-code candidate v0."""
...
import dynamic_floquet_code_candidate_v0 as candidate  # noqa: E402


class DynamicFloquetCodeCandidateV0Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_candidate_is_killed_by_fixed_code_schedule(self) -> None:
        self.assertEqual(
    ...
    def test_logical_sector_signal_is_not_source_growth(self) -> None:
        self.assertTrue(self.result["logical_sector_signal_present"])
        self.assertTrue(
            self.gates["W1_non_isomorphic_source_algebra"]["logical_signal"]
        )
        self.assertTrue(
            self.gates["W2_new_admissibility_predicate"]["logical_signal"]
        )
        self.assertTrue(
            self.gates["W3_construction_space_growth"]["logical_signal"]
        )
        self.assertFalse(
    ...
    def test_fixed_source_rival_preserves_trace_without_source_law(self) -> None:
        ...
        self.assertTrue(rival["fixed_physical_hilbert_space"])
        self.assertTrue(rival["fixed_stabilizer_or_gauge_family"])
        self.assertTrue(rival["fixed_time_periodic_measurement_schedule"])
        self.assertTrue(rival["fixed_boundary_and_initialization_data"])
        self.assertTrue(rival["fixed_decoder_and_logical_readout_rule"])
        self.assertTrue(rival["noise_or_syndrome_random_tape"])
        self.assertTrue(rival["completed_syndrome_history_available"])
        self.assertTrue(rival["record_trace_preserved"])
        self.assertTrue(rival["reproduces_candidate_shape"])
        self.assertFalse(rival["source_owned_new_code_law"])

    def test_missing_source_object_names_resurrection_burden(self) -> None:
        missing = self.result["exact_missing_source_object"]
        self.assertEqual(
            missing["missing_object_id"], "source_owned_code_space_generation_law"
        )
        self.assertIn("native physical source law", missing["required"])
        self.assertIn("fixed Hilbert space", missing["required"])
