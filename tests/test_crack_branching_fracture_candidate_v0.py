#!/usr/bin/env python3
"""Focused tests for crack branching fracture candidate v0."""
...
import crack_branching_fracture_candidate_v0 as candidate  # noqa: E402


class CrackBranchingFractureCandidateV0Tests(unittest.TestCase):
    def setUp(self) -> None:
        self.result = candidate.run_candidate()
        self.gates = {row["gate_id"]: row for row in self.result["witness_gates"]}

    def test_candidate_is_killed_by_fixed_fracture_mechanics(self) -> None:
    ...
    def test_fracture_signal_is_not_source_growth(self) -> None:
        self.assertTrue(self.result["fracture_signal_present"])
        self.assertTrue(
            self.gates["W1_non_isomorphic_source_algebra"]["fracture_signal"]
        )
        self.assertTrue(
            self.gates["W2_new_admissibility_predicate"]["fracture_signal"]
        )
        self.assertTrue(
            self.gates["W3_construction_space_growth"]["fracture_signal"]
        )
        self.assertFalse(
    ...
    def test_fixed_source_rival_preserves_trace_without_source_law(self) -> None:
        ...
        self.assertTrue(rival["fixed_elastodynamic_or_phase_field_law"])
        self.assertTrue(rival["fixed_material_microstructure"])
        self.assertTrue(rival["fixed_initial_flaw_distribution"])
        self.assertTrue(rival["fixed_specimen_geometry_and_boundaries"])
        self.assertTrue(rival["fixed_loading_history_and_rate"])
        self.assertTrue(rival["fixed_toughness_and_energy_release_criterion"])
        self.assertTrue(rival["fixed_process_zone_and_dissipation_model"])
        self.assertTrue(rival["stochastic_disorder_tape"])
        self.assertTrue(rival["completed_fracture_history_available"])
        self.assertTrue(rival["record_trace_preserved"])
        self.assertTrue(rival["reproduces_candidate_shape"])
        self.assertFalse(rival["source_owned_fracture_law"])
    ...
    def test_missing_source_object_names_resurrection_burden(self) -> None:
        ...
        self.assertEqual(
            ...
            "source_owned_fracture_interface_generation_law",
        )
        self.assertIn("native physical source law", missing["required"])
        self.assertIn("fixed elastodynamics", missing["required"])
