#!/usr/bin/env python3
"""Focused tests for r-process nucleosynthesis candidate v0."""
...
import r_process_nucleosynthesis_candidate_v0 as candidate  # noqa: E402


class RProcessNucleosynthesisCandidateV0Tests(unittest.TestCase):
    def setUp(self) -> None:
        self.result = candidate.run_candidate()
        self.gates = {row["gate_id"]: row for row in self.result["witness_gates"]}

    def test_candidate_is_killed_by_fixed_nuclear_reaction_network(self) -> None:
    ...
    def test_nucleosynthesis_signal_is_not_source_growth(self) -> None:
        self.assertTrue(self.result["nucleosynthesis_signal_present"])
        self.assertTrue(
            self.gates["W1_non_isomorphic_source_algebra"][
                "nucleosynthesis_signal"
            ]
        )
        self.assertTrue(
            self.gates["W2_new_admissibility_predicate"]["nucleosynthesis_signal"]
        )
        self.assertTrue(
            self.gates["W3_construction_space_growth"]["nucleosynthesis_signal"]
        )
        self.assertFalse(
    ...
    def test_fixed_source_rival_preserves_trace_without_source_law(self) -> None:
        ...
        self.assertTrue(rival["fixed_nuclear_species_graph"])
        self.assertTrue(rival["fixed_nuclear_mass_and_rate_tables"])
        self.assertTrue(rival["fixed_seed_abundance"])
        self.assertTrue(rival["fixed_neutron_flux_and_electron_fraction"])
        self.assertTrue(rival["fixed_thermodynamic_expansion_trajectory"])
        self.assertTrue(rival["fixed_astrophysical_ejecta_context"])
        self.assertTrue(rival["fixed_fission_recycling_channels"])
        self.assertTrue(rival["stochastic_ejecta_or_rate_uncertainty_tape"])
        self.assertTrue(rival["completed_abundance_history_available"])
        self.assertTrue(rival["record_trace_preserved"])
        self.assertTrue(rival["reproduces_candidate_shape"])
        self.assertFalse(rival["source_owned_species_law"])
    ...
    def test_missing_source_object_names_resurrection_burden(self) -> None:
        ...
        self.assertEqual(
            ...
            "source_owned_nuclear_species_generation_law",
        )
        self.assertIn("native physical source law", missing["required"])
        self.assertIn("fixed nuclear", missing["required"])
