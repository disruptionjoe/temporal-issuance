#!/usr/bin/env python3
"""Focused tests for Schwinger pair-production candidate v0."""
...
import schwinger_pair_production_candidate_v0 as candidate  # noqa: E402


class SchwingerPairProductionCandidateV0Tests(unittest.TestCase):
    def setUp(self) -> None:
        self.result = candidate.run_candidate()
        self.gates = {row["gate_id"]: row for row in self.result["witness_gates"]}

    def test_candidate_is_killed_by_fixed_qft_background(self) -> None:
    ...
    def test_pair_signal_is_not_source_growth(self) -> None:
        self.assertTrue(self.result["pair_production_signal_present"])
        self.assertTrue(
            self.gates["W1_non_isomorphic_source_algebra"]["pair_signal"]
        )
        self.assertTrue(self.gates["W2_new_admissibility_predicate"]["pair_signal"])
        self.assertTrue(self.gates["W3_construction_space_growth"]["pair_signal"])
        self.assertFalse(
    ...
    def test_fixed_source_rival_preserves_trace_without_source_law(self) -> None:
        ...
        self.assertTrue(rival["fixed_lagrangian"])
        self.assertTrue(rival["fixed_charged_field_content"])
        self.assertTrue(rival["fixed_background_field_profile"])
        self.assertTrue(rival["fixed_boundary_conditions"])
        self.assertTrue(rival["fixed_vacuum_state"])
        self.assertTrue(rival["fixed_couplings_and_renormalization"])
        self.assertTrue(rival["stochastic_vacuum_amplitude_or_mode_tape"])
        self.assertTrue(rival["completed_field_history_available"])
        self.assertTrue(rival["record_trace_preserved"])
        self.assertTrue(rival["reproduces_candidate_shape"])
        self.assertFalse(rival["source_owned_new_field_law"])
    ...
    def test_missing_source_object_names_resurrection_burden(self) -> None:
        ...
        self.assertEqual(
            ...
            "source_owned_field_algebra_expansion_law",
        )
        self.assertIn("native physical source law", missing["required"])
        self.assertIn("fixed QFT", missing["required"])
