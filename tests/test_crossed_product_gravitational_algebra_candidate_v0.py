#!/usr/bin/env python3
"""Focused tests for crossed-product gravitational algebra candidate v0."""
...
import crossed_product_gravitational_algebra_candidate_v0 as candidate  # noqa: E402


class CrossedProductGravitationalAlgebraCandidateV0Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_candidate_is_killed_by_fixed_gravitational_bookkeeping(self) -> None:
        self.assertEqual(
    ...
    def test_algebra_signal_is_not_source_growth(self) -> None:
        self.assertTrue(self.result["algebra_signal_present"])
        self.assertTrue(
            self.gates["W1_non_isomorphic_source_algebra"]["algebra_signal"]
        )
        self.assertFalse(
    ...
    def test_w4_and_w6_absorber_controls_fail_while_w5_survives(self) -> None:
        ...
        self.assertFalse(
            self.gates["W6_observer_algebra_bookkeeping_absorption"][
                "source_passed"
            ]
        )
    ...
    def test_fixed_source_rival_preserves_trace_without_source_law(self) -> None:
        ...
        self.assertTrue(rival["fixed_global_theory_or_hamiltonian"])
        self.assertTrue(rival["fixed_state_or_path_integral_family"])
        self.assertTrue(rival["fixed_observer_frame_or_dressing"])
        self.assertTrue(rival["fixed_crossed_product_or_modular_flow_recipe"])
        self.assertTrue(rival["fixed_boundary_or_constraint_data"])
        self.assertTrue(rival["completed_history_available"])
        self.assertTrue(rival["record_trace_preserved"])
        self.assertTrue(rival["reproduces_candidate_shape"])
        self.assertFalse(rival["source_owned_new_algebra_law"])
    ...
    def test_missing_source_object_names_resurrection_burden(self) -> None:
        ...
        self.assertEqual(
            ...
            "source_owned_gravitational_algebra_transition_law",
        )
