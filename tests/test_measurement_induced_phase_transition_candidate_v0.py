#!/usr/bin/env python3
"""Focused tests for measurement-induced phase transition candidate v0."""
...
import measurement_induced_phase_transition_candidate_v0 as candidate  # noqa: E402


class MeasurementInducedPhaseTransitionCandidateV0Tests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_candidate_is_killed_by_fixed_monitored_dynamics(self) -> None:
        self.assertEqual(
    ...
    def test_transition_signal_is_not_source_growth(self) -> None:
        self.assertTrue(self.result["transition_signal_present"])
        self.assertTrue(
            self.gates["W1_non_isomorphic_source_law"]["transition_signal"]
        )
        self.assertFalse(
            self.gates["W1_non_isomorphic_source_law"]["source_passed"]
        )
    ...
    def test_w4_and_w6_absorber_controls_fail_while_w5_survives(self) -> None:
        ...
        self.assertFalse(
            self.gates["W6_access_history_absorption"]["source_passed"]
        )
    ...
    def test_fixed_source_rival_preserves_trace_without_source_law(self) -> None:
        ...
        self.assertTrue(rival["fixed_dynamics_family"])
        self.assertTrue(rival["fixed_measurement_or_control_schedule"])
        self.assertTrue(rival["stochastic_trajectory_or_born_random_tape"])
        self.assertTrue(rival["postselection_or_observer_access_channel"])
        self.assertTrue(rival["completed_trajectory_history_available"])
        self.assertTrue(rival["record_trace_preserved"])
        self.assertTrue(rival["reproduces_candidate_shape"])
        self.assertFalse(rival["source_owned_transition_law"])

    def test_missing_source_object_names_resurrection_burden(self) -> None:
        missing = self.result["exact_missing_source_object"]
        self.assertEqual(
            missing["missing_object_id"], "source_owned_monitoring_transition_law"
        )
