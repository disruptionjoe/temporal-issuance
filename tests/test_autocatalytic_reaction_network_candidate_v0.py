#!/usr/bin/env python3
"""Focused tests for autocatalytic reaction-network candidate v0."""
...
import autocatalytic_reaction_network_candidate_v0 as candidate  # noqa: E402
import completion_class_v1 as cc  # noqa: E402


class AutocatalyticReactionNetworkCandidateV0Tests(unittest.TestCase):
    def setUp(self) -> None:
        self.result = candidate.run_candidate()
        self.gates = {row["gate_id"]: row for row in self.result["witness_gates"]}

    def test_candidate_is_killed_by_fixed_reaction_network(self) -> None:
    ...
    def test_autocatalytic_signal_is_not_source_growth(self) -> None:
        self.assertTrue(self.result["autocatalytic_signal_present"])
        self.assertTrue(
            self.gates["W1_non_isomorphic_source_algebra"]["network_signal"]
        )
        self.assertTrue(
            self.gates["W2_new_admissibility_predicate"]["network_signal"]
        )
        self.assertTrue(
            self.gates["W3_construction_space_growth"]["network_signal"]
        )
        self.assertFalse(
    ...
    def test_fixed_source_rival_preserves_trace_without_source_law(self) -> None:
        ...
        self.assertTrue(rival["fixed_species_alphabet"])
        self.assertTrue(rival["fixed_reaction_graph"])
        self.assertTrue(rival["fixed_catalyst_inventory"])
        self.assertTrue(rival["fixed_rate_law_table"])
        self.assertTrue(rival["fixed_feedstock_and_boundary_flux"])
        self.assertTrue(rival["seed_concentrations_as_initial_condition"])
        self.assertTrue(rival["stochastic_collision_or_reaction_tape"])
        self.assertTrue(rival["completed_reaction_history_available"])
        self.assertTrue(rival["record_trace_preserved"])
        self.assertTrue(rival["reproduces_candidate_shape"])
        self.assertFalse(rival["source_owned_new_reaction_law"])
    ...
    def test_missing_source_object_names_resurrection_burden(self) -> None:
        ...
        self.assertEqual(
            ...
            "source_owned_reaction_grammar_expansion_law",
        )
        self.assertIn("native physical source law", missing["required"])
        self.assertIn("fixed reaction graph", missing["required"])
