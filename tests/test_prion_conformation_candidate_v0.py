#!/usr/bin/env python3
"""Focused tests for prion conformation candidate v0."""
...
import prion_conformation_candidate_v0 as candidate  # noqa: E402


class PrionConformationCandidateV0Tests(unittest.TestCase):
    def setUp(self) -> None:
        self.result = candidate.run_candidate()
        self.gates = {row["gate_id"]: row for row in self.result["witness_gates"]}

    def test_candidate_is_killed_by_fixed_conformational_landscape(self) -> None:
    ...
    def test_conformation_signal_is_not_source_growth(self) -> None:
        self.assertTrue(self.result["conformation_signal_present"])
        self.assertTrue(
            self.gates["W1_non_isomorphic_source_algebra"]["conformation_signal"]
        )
        self.assertTrue(
            self.gates["W2_new_admissibility_predicate"]["conformation_signal"]
        )
        self.assertTrue(
            self.gates["W3_construction_space_growth"]["conformation_signal"]
        )
        self.assertFalse(
    ...
    def test_fixed_source_rival_preserves_trace_without_source_law(self) -> None:
        ...
        self.assertTrue(rival["fixed_amino_acid_sequence"])
        self.assertTrue(rival["fixed_conformational_energy_landscape"])
        self.assertTrue(rival["fixed_solvent_temperature_and_chaperone_context"])
        self.assertTrue(rival["seed_conformer_as_initial_condition"])
        self.assertTrue(rival["stochastic_collision_or_folding_tape"])
        self.assertTrue(rival["assay_and_strain_labeling_rule"])
        self.assertTrue(rival["completed_folding_history_available"])
        self.assertTrue(rival["record_trace_preserved"])
        self.assertTrue(rival["reproduces_candidate_shape"])
        self.assertFalse(rival["source_owned_new_conformation_law"])
    ...
    def test_missing_source_object_names_resurrection_burden(self) -> None:
        ...
        self.assertEqual(
            ...
            "source_owned_conformational_grammar_expansion_law",
        )
        self.assertIn("native physical source law", missing["required"])
        self.assertIn("fixed conformational landscape", missing["required"])
