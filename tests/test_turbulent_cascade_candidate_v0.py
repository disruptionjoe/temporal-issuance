#!/usr/bin/env python3
"""Focused tests for turbulent cascade candidate v0."""
...
import turbulent_cascade_candidate_v0 as candidate  # noqa: E402


class TurbulentCascadeCandidateV0Tests(unittest.TestCase):
    def setUp(self) -> None:
        self.result = candidate.run_candidate()
        self.gates = {row["gate_id"]: row for row in self.result["witness_gates"]}

    def test_candidate_is_killed_by_fixed_fluid_cascade(self) -> None:
    ...
    def test_turbulence_signal_is_not_source_growth(self) -> None:
        self.assertTrue(self.result["turbulence_signal_present"])
        self.assertTrue(
            self.gates["W1_non_isomorphic_source_algebra"]["turbulence_signal"]
        )
        self.assertTrue(
            self.gates["W2_new_admissibility_predicate"]["turbulence_signal"]
        )
        self.assertTrue(
            self.gates["W3_construction_space_growth"]["turbulence_signal"]
        )
        self.assertFalse(
    ...
    def test_fixed_source_rival_preserves_trace_without_source_law(self) -> None:
        ...
        self.assertTrue(rival["fixed_governing_equations"])
        self.assertTrue(rival["fixed_fluid_parameters"])
        self.assertTrue(rival["fixed_domain_and_boundary_conditions"])
        self.assertTrue(rival["fixed_initial_conditions"])
        self.assertTrue(rival["fixed_forcing_and_energy_injection"])
        self.assertTrue(rival["fixed_reynolds_regime_and_scale_window"])
        self.assertTrue(rival["fixed_subgrid_or_closure_family"])
        self.assertTrue(rival["stochastic_perturbation_tape"])
        self.assertTrue(rival["completed_flow_history_available"])
        self.assertTrue(rival["record_trace_preserved"])
        self.assertTrue(rival["reproduces_candidate_shape"])
        self.assertFalse(rival["source_owned_cascade_law"])
    ...
    def test_missing_source_object_names_resurrection_burden(self) -> None:
        ...
        self.assertEqual(
            ...
            "source_owned_cascade_rule_generation_law",
        )
        self.assertIn("native physical source law", missing["required"])
        self.assertIn("fixed fluid equations", missing["required"])
