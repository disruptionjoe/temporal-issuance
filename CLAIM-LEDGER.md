...
TI-C019:
  ...
  current_strongest_version: >
    D4 and OnlineSchemaSys remain valid formal tools for projection/process novelty, but
    source-side issuance now requires a witness that cannot be represented as bounded access
    to a fixed richer source.
  current_strongest_objection: >
    A static `Mu_infty` plus access apertures `P_n` can reproduce any finite projection-level
    D4 trace without source schema expansion.
  evidence_refs_added:
  ...
  next_action: >
    Run a source-side witness fixture: AC-8 quorum/adversary, sheaf/holonomy with source-side
    compatibility predicate, or grokking-vs-self-play as projection/source discriminator.
TI-C020:
  status: speculative
  effect: pressured
  current_objection: >
    Observer-level OnlineSchemaSys behavior can arise from bounded access to a fixed richer
    source, so physical-universe-as-OnlineSchemaSys requires direct source-layer evidence.
  evidence_refs_added:
    - explorations/E029-bounded-accessibility-source-projection-model.md
