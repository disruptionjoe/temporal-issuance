---
artifact_type: stewardship_rules
status: active
governance_role: operating_rules
constitutional: false
---

# Stewardship Rules

1. Research verdict outranks governance elegance.
2. Governance mechanisms are hypotheses.
3. Preserve provenance.
4. Prefer append-only history where possible.
5. Do not hide state outside the repository.
6. Do not treat absorption as automatically terminal.
7. Test category and layer errors.
8. Record killed paths with resurrection triggers.
9. Keep human review uncertainty-driven.
10. Build the strongest surviving version before attempting destruction.

